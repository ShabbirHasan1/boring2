use futures::future;
use rboring::ssl::{SslAcceptor, SslConnector, SslFiletype, SslMethod};
use std::future::Future;
use std::net::{SocketAddr, ToSocketAddrs};
use std::pin::Pin;
use tokio::io::{AsyncReadExt, AsyncWrite, AsyncWriteExt};
use tokio::net::{TcpListener, TcpStream};
use tokio_rboring::{HandshakeError, SslStream};
...
async fn google() {
    ...
    let config = SslConnector::builder(SslMethod::tls())
        ...
        .configure()
        .unwrap();
    let mut stream = tokio_rboring::connect(config, "google.com", stream)
        .await
        .unwrap();
...
fn create_server() -> (
    ...
    let server = async move {
        ...
        tokio_rboring::accept(&acceptor, stream).await
    };
...
async fn server() {
    ...
    let client = async {
        ...
        let stream = TcpStream::connect(&addr).await.unwrap();
        let mut stream = tokio_rboring::connect(config, "localhost", stream)
            .await
            .unwrap();
...
async fn handshake_error() {
    ...
    let client = async {
        ...
        let err = tokio_rboring::connect(config, "localhost", stream)
            .await
            .unwrap_err();
